...
pub fn get_tools_list() -> Result<serde_json::Value, JsonRpcError> {
    let tools = vec![
        ...
        // ── Phase 8: New tools ──────────────────────────────────────────────
        Tool {
            name: "cwa_get_tech_stack".to_string(),
            description: "Get the project's technology stack (languages, frameworks, databases). Use this to understand which tech agents were generated by 'cwa codegen all'.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {}
            }),
        ...
        Tool {
            name: "cwa_cache_status".to_string(),
            description: "Show Redis cache status: total key count and memory usage. Useful for understanding CWA data volume.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {}
            }),
        ...
        Tool {
            name: "cwa_graph_hyperedges".to_string(),
            description: "List Neo4j hyperedges for a given entity. Hyperedges model n-ary relationships (e.g. a spec that involves a context + domain object + decision).".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "entity_type": {
                        "type": "string",
                        "description": "Entity type: Project, Spec, Task, BoundedContext, DomainObject, Observation, File, Decision"
                    },
                    "entity_id": {
                        "type": "string",
                        "description": "Entity ID"
        ...
        Tool {
            name: "cwa_codegen_agents".to_string(),
            description: "List which tech-stack agent templates would be generated by 'cwa codegen all' for this project's tech stack. Does not write files — run 'cwa codegen all' in the CLI to generate.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {}
            }),
        ...
        Tool {
            name: "cwa_hybrid_search".to_string(),
            description: "Hybrid semantic search across all CWA vector collections (specs, tasks, domain, memory, files). Combines dense vector search with keyword filtering via RRF fusion.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "query": {
                    ...
                    "collections": {
                        "type": "array",
                        "items": { "type": "string" },
                        "description": "Collections to search: cwa_specs, cwa_tasks, cwa_domain, cwa_memory, cwa_files (default: all)"
                    }
                },
                "required": ["query"]
            }),
...
pub async fn call_tool(
    ...
    let result = match name {
        ...
        // ── Phase 8: New tools ──────────────────────────────────────────────
        "cwa_get_tech_stack" => {
            let stack = cwa_db::queries::projects::get_tech_stack(pool, &project.id).await
                .unwrap_or_default();

            serde_json::json!({
                "tech_stack": stack,
                "count": stack.len(),
                "hint": "Run 'cwa update' to modify. Run 'cwa codegen all' to generate tech agents."
            })
        }

        "cwa_cache_status" => {
            // Use redis INFO MEMORY and DBSIZE to report cache health
        ...
        "cwa_graph_hyperedges" => {
            let entity_type_str = args["entity_type"].as_str().ok_or_else(|| JsonRpcError {
                code: -32602,
                message: "Missing entity_type".to_string(),
            })?;
            let entity_id = args["entity_id"].as_str().ok_or_else(|| JsonRpcError {
            ...
            let entity_type = cwa_graph::EntityType::from_str(entity_type_str);
            match cwa_graph::GraphClient::connect_default().await {
                Ok(graph) => {
                    let edges = cwa_graph::find_hyperedges_for_entity(&graph, entity_type, entity_id).await
                        .map_err(|e| JsonRpcError {
                            code: -32603,
                            message: e.to_string(),
                        })?;
                ...
                Err(_) => {
                    serde_json::json!({
                        ...
                        "hyperedge_count": 0,
                        "hyperedges": [],
                        "warning": "Neo4j not available — run 'cwa infra up' to start the graph database"
                    })
                }
            }
        }
        ...
        "cwa_codegen_agents" => {
            let tech_stack = cwa_db::queries::projects::get_tech_stack(pool, &project.id).await
                .unwrap_or_default();
            let agents = cwa_codegen::select_agents_for_stack(&tech_stack);
            let agent_names: Vec<&str> = agents.iter().map(|a| a.filename.as_str()).collect();

            serde_json::json!({
                "tech_stack": tech_stack,
                "agent_count": agents.len(),
                "agents": agent_names,
                "hint": "Run 'cwa codegen all' in your project directory to write these files to .claude/agents/"
            })
        }
        ...
        "cwa_hybrid_search" => {
            ...
            let collections: Vec<String> = args.get("collections")
                .and_then(|v| v.as_array())
                .map(|arr| arr.iter().filter_map(|v| v.as_str().map(str::to_string)).collect())
                .unwrap_or_else(|| cwa_embedding::QdrantStore::all_collections().iter().map(|s| s.to_string()).collect());

            let ollama_url = std::env::var("OLLAMA_URL").unwrap_or_else(|_| "http://localhost:11434".to_string());
            let qdrant_url = std::env::var("QDRANT_URL").unwrap_or_else(|_| "http://localhost:6333".to_string());

            match (
                cwa_embedding::QdrantStore::new(&qdrant_url),
            ) {
                (Ok(qdrant),) => {
                    let ollama = cwa_embedding::OllamaClient::new(&ollama_url, "nomic-embed-text");
                    let req = cwa_embedding::HybridSearchRequest {
                        query,
                        top_k,
                        collections,
                        project_id: Some(&project.id),
                        fusion: cwa_embedding::FusionAlgo::Rrf,
                    };
                    match cwa_embedding::hybrid_search(&ollama, &qdrant, req).await {
                        Ok(results) => serde_json::json!({
                            "query": query,
                            "count": results.len(),
                            "results": results
                        }),
                        Err(e) => return Ok(tool_error(&format!("Hybrid search failed: {}", e))),
                    }
                }
                _ => {
                    return Ok(tool_error("Qdrant not available — run 'cwa infra up' to start the vector database"));
                }
            }
        }
...
pub fn get_resources_list() -> Result<serde_json::Value, JsonRpcError> {
    let resources = vec![
        ...
        Resource {
            uri: "project://tech-stack".to_string(),
            name: "Tech Stack".to_string(),
            description: "Project technology stack: languages, frameworks, databases, and tools".to_string(),
            mime_type: "application/json".to_string(),
        },
    ];
