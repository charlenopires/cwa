...
pub async fn create_spec(
    ...
    create_spec_with_criteria(pool, project_id, title, description, priority, None).await
}
...
pub async fn create_spec_with_criteria(
    ...
    if let Some(criteria) = criteria {
        ...
        queries::create_spec_with_criteria(pool, &id, project_id, title, description, priority, &criteria_json).await?;
    } else {
        queries::create_spec(pool, &id, project_id, title, description, priority).await?;
    }

    let row = queries::get_spec(pool, &id).await?;
    Ok(Spec::from_row(row))
}

/// Get a spec by ID, ID prefix, or title.
pub async fn get_spec(pool: &DbPool, project_id: &str, identifier: &str) -> CwaResult<Spec> {
    // Try exact ID first
    match queries::get_spec(pool, identifier).await {
        Ok(row) => return Ok(Spec::from_row(row)),
        Err(cwa_db::DbError::NotFound(_)) => {}
        Err(e) => return Err(e.into()),
    }
    // Try ID prefix match
    match queries::get_spec_by_id_prefix(pool, identifier).await {
        Ok(row) => return Ok(Spec::from_row(row)),
        Err(cwa_db::DbError::NotFound(_)) => {}
        Err(e) => return Err(e.into()),
    }
    // Try by title
    let row = queries::get_spec_by_title(pool, project_id, identifier).await
        .map_err(|_| CwaError::SpecNotFound(identifier.to_string()))?;
    Ok(Spec::from_row(row))
}

/// List all specs for a project.
pub async fn list_specs(pool: &DbPool, project_id: &str) -> CwaResult<Vec<Spec>> {
    let rows = queries::list_specs(pool, project_id).await?;
    Ok(rows.into_iter().map(Spec::from_row).collect())
}

/// Get the active spec for a project.
pub async fn get_active_spec(pool: &DbPool, project_id: &str) -> CwaResult<Option<Spec>> {
    let row = queries::get_active_spec(pool, project_id).await?;
    Ok(row.map(Spec::from_row))
}

/// Update spec status.
pub async fn update_status(pool: &DbPool, id: &str, status: &str) -> CwaResult<()> {
    let current = queries::get_spec(pool, id).await?;
    ...
    queries::update_spec_status(pool, id, status).await?;
    Ok(())
}

/// Archive a spec.
pub async fn archive_spec(pool: &DbPool, id: &str) -> CwaResult<()> {
    update_status(pool, id, "archived").await
}

/// Clear all specs for a project. Returns the number of deleted specs.
pub async fn clear_specs(pool: &DbPool, project_id: &str) -> CwaResult<usize> {
    let count = queries::delete_all_specs(pool, project_id).await?;
    Ok(count)
}
...
pub async fn add_acceptance_criteria(
    ...
    let spec = get_spec(pool, project_id, identifier).await?;
    ...
    let criteria_json = serde_json::to_string(&criteria)
        .map_err(|e| CwaError::ValidationError(format!("Failed to serialize criteria: {}", e)))?;
    queries::update_acceptance_criteria(pool, &spec.id, &criteria_json).await?;

    let row = queries::get_spec(pool, &spec.id).await?;
    Ok(Spec::from_row(row))
}
...
pub async fn create_specs_from_prompt(
    pool: &DbPool,
    project_id: &str,
    input: &str,
    priority: &str,
    ...
    for entry in parsed {
        ...
        let spec = create_spec(pool, project_id, &entry.title, entry.description.as_deref(), prio).await?;
        specs.push(spec);
    }
